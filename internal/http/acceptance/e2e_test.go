...
const (
	testDatabaseURL = "postgres://postgres:postgres@localhost:5433/bonsai_test?sslmode=disable"
	testRedisURL    = "redis://localhost:6380/1" // Use different port and DB 1 for tests
)
...
func startServices() error {
	cmd := exec.Command("docker", "compose", "-f", "docker/docker-compose.test.yaml", "up", "-d")
	cmd.Dir = "../../../" // Set working directory to project root
	out, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to start test services: %v\nOutput: %s\n", err, out)
	}
	return err
}

func stopServices() {
	cmd := exec.Command("docker", "compose", "-f", "docker/docker-compose.test.yaml", "down", "-v")
	cmd.Dir = "../../../" // Set working directory to project root
	cmd.Run()
}
...
func waitForServices() error {
	...
	// Wait for Redis
	for i := 0; i < 30; i++ {
		rdb := redis.NewClient(&redis.Options{Addr: "localhost:6380", DB: 1})
...
func createTestDatabase() error {
	...
	defaultURL := "postgres://postgres:postgres@localhost:5433/postgres?sslmode=disable"
...
func TestE2E_SnippetCRUD(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_SnippetValidation(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_ListPaginationAndFiltering(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_HeaderPropagation(t *testing.T) {
...
func TestE2E_ExpiredSnippets(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_DatabaseConsistency(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_RedisFailover(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_PaginationConsistency(t *testing.T) {
	cleanDatabase(t)
...
func TestE2E_PerformanceAndLoad(t *testing.T) {
	if testing.Short() {
