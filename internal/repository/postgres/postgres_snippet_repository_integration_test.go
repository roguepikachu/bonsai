...
func TestPostgresRepository_CRUDAndList(t *testing.T) {
    ...
    // Insert snippets
    if err := repo.Insert(ctx, s1); err != nil { t.Fatalf("insert s1: %v", err) }
    if err := repo.Insert(ctx, s2); err != nil { t.Fatalf("insert s2: %v", err) }
    if err := repo.Insert(ctx, s3); err != nil { t.Fatalf("insert s3: %v", err) }

    // FindByID
    got, err := repo.FindByID(ctx, "a1")
    if err != nil { t.Fatalf("find a1: %v", err) }
    if got.ID != "a1" || got.Content != s1.Content { t.Fatalf("find mismatch: %+v", got) }

    // List all (order by created_at desc)
    all, err := repo.List(ctx, 1, 10, "")
    if err != nil { t.Fatalf("list all: %v", err) }
    if len(all) != 3 { t.Fatalf("want 3, got %d", len(all)) }
    ...
    if err != nil { t.Fatalf("list go: %v", err) }
    if len(goOnly) != 2 { t.Fatalf("want 2 go-tagged, got %d", len(goOnly)) }

    // Pagination
    page1, err := repo.List(ctx, 1, 2, "")
    if err != nil { t.Fatalf("list page1: %v", err) }
    page2, err := repo.List(ctx, 2, 2, "")
    if err != nil { t.Fatalf("list page2: %v", err) }
    if len(page1) != 2 || len(page2) != 1 { t.Fatalf("pagination wrong: p1=%d p2=%d", len(page1), len(page2)) }
}
...
func domainSnippet(id string, created time.Time, expires *time.Time, tags []string) domain.Snippet {
    ...
    if expires != nil { s.ExpiresAt = *expires }
    return s
}
